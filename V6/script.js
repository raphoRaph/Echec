...
function echec_et_mat(){
    if(pions_echec.length == 1){
        ...
        for(let i = 0; i < cases_possibles_deplacement.length; i++){
            ...
            if(! echec_toutes_directions(p, petit_echec)){
                return
            }
        }
        p.frame = fr
...
function petit_echec(roi, liste){
	...
	for(let i = 1; i < liste.length; i++){
		if(liste[i].pion){
            /*
            if(petit_echec.caller.caller.name == "echec_et_mat"){
                console.log(roi, liste)
                console.log(!met_en_echec(roi, liste[1], liste[0]))
            }*/
            // la case n'est pas mise en echec
			if(liste[i].pion.id[0] == roi.id[0] || !met_en_echec(roi, liste[i], liste[0])){
                return false
			}
            // la case est mise en echec
            /*
            if(petit_echec.caller.caller.name == "echec_et_mat"){
                console.log("no")
            }
...
function met_en_echec(roi, frame, dir){
    /*
    if(met_en_echec.caller.caller.caller.name == "echec_et_mat"){
        console.log(roi, frame.pion.id, dir)
    }
    */
